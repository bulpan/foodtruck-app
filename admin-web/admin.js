                ...
                row.innerHTML = `
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            <span class="text-muted small me-2" title="${token.token}">
                                ${token.token.substring(0, 10)}...${token.token.substring(token.token.length - 10)}
                            </span>
                            <button class="btn btn-sm btn-outline-secondary" onclick="navigator.clipboard.writeText('${token.token}').then(() => showAlert('토큰이 복사되었습니다.', 'success'))">
